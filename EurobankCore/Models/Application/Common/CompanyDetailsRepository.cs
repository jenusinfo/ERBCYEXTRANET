...
namespace Eurobank.Models.Application.Common
{
    public class CompanyDetailsRepository
    {
        ...
        public CompanyDetailsRepository(IPageRetriever pageRetriever, IPageDataContextRetriever pageDataContextRetriever)
        {
        ...
        public Task<IEnumerable<CMS.DocumentEngine.Types.Eurobank.CompanyDetails>> GetCompanyDetailsAsync(string nodeAliasPath, CancellationToken cancellationToken)
        {
            return pageRetriever.RetrieveAsync<CMS.DocumentEngine.Types.Eurobank.CompanyDetails>(
                query => query
                    .Path(nodeAliasPath, PathTypeEnum.Children)
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(CompanyDetails)}|{nameof(GetCompanyDetailsAsync)}|{nodeAliasPath}")
                    // Include path dependency to flush cache when a new child page is created or page order is changed.
                    .Dependencies((_, builder) => builder.PagePath(nodeAliasPath, PathTypeEnum.Children).PageOrder()),
                cancellationToken);
        }
        public IEnumerable<CMS.DocumentEngine.Types.Eurobank.CompanyDetails> GetCompanyDetails(int apllicationID)
        {
            var apllicationDetails = pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.CompanyDetails>(
                 query => query
                     .OrderBy("NodeOrder")
                     .WhereEquals("CompanyDetailsID", apllicationID)
                ).FirstOrDefault();
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.CompanyDetails>(
                query => query
                    ...
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(CompanyDetailsRepository)}|{nameof(CompanyDetails)}|{apllicationDetails.NodeAliasPath}")
                    // Include path dependency to flush cache when a new child page is created or page order is changed.
        ...
        public CMS.DocumentEngine.Types.Eurobank.CompanyDetails GetCompanyDetailsByID(int CompanyDetailsID)
        {
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.CompanyDetails>(
                query => query
                    .OrderBy("NodeOrder")
                    .WhereEquals("CompanyDetailsID", CompanyDetailsID)
               ).FirstOrDefault();
        }
