...
public partial class CMSModules_Membership_Controls_Users_UserFilter : CMSAbstractBaseFilterControl, IUserFilter
{
    ...
    public override void ResetFilter()
    {
        ...
        ddlUserTypes_SelectedIndexChanged(ddlUserTypes, null);
        // ---------------------------------------------------------------------------------------------------
    }
    ...
    private WhereCondition GetAdvancedSearchCondition()
    {
        ...
        // Custom
        // ---------------------------------------------------------------------------------------------------
        AddUserTypeCondition(whereCondition);
        AddUserOrganisationCondition(whereCondition);
        // ---------------------------------------------------------------------------------------------------
    ...
    private void AddItemsUserTypes()
	{
        ddlUserTypes.Items.Add(new ListItem() { Text = "(any)", Value = "_any" });

        var userTypes = LookupItemProvider.GetLookupItems()
            .Path("/Lookups/General/ENTITIES", CMS.DocumentEngine.PathTypeEnum.Children)
            .Columns("NodeGUID, NodeName")
            .OrderBy("NodeOrder");

        foreach(var item in userTypes)
		{
            ddlUserTypes.Items.Add(new ListItem() { Text = item.NodeName, Value = item.NodeGUID.ToString() });
		}
	}


    private void AddItemsUserOrganisations(string userTypeGUID)
    {
        ddlUserOrganisations.Items.Add(new ListItem() { Text = "(any)", Value = "_any" });

        string path = null, className = null;
        switch(userTypeGUID)
		{
            case "190c271e-bbfd-47b7-9824-bff95924f085": // Internal
            case "e89527b3-187e-4b7d-8a09-f73ef0158f3f": // Corporate
                path = "/Bank-Units";
                className = "Eurobank.BankUnit";
                break;
            case "1f421143-6571-4122-9e9f-3a7c209f49eb": // Introducer
                path = "/Intermediaries";
                className = "Eurobank.Intermediary";
                break;
            default:
                break;
        }
        ...
        var userOrganisations = DocumentHelper.GetDocuments(className)
        ...
        {
            ddlUserOrganisations.Items.Add(new ListItem() { Text = item.NodeName, Value = item.NodeGUID.ToString() });
        }
    }
    ...
    {
        ddlUserOrganisations.Items.Clear();
        AddItemsUserOrganisations((sender as CMSDropDownList).SelectedValue);
    }


    private void AddUserTypeCondition(WhereCondition whereCondition)
	{
        if(ddlUserTypes.SelectedValue == "_any") return;

        var query = UserSettingsInfo.Provider.Get()
           .Column("UserSettingsUserID")
           .Where($"UserType='{ddlUserTypes.SelectedValue}'");

        whereCondition.WhereIn("UserId", query);
    }


    private void AddUserOrganisationCondition(WhereCondition whereCondition)
	{
        if(ddlUserOrganisations.SelectedValue == "_any") return;

        var query = UserSettingsInfo.Provider.Get()
           .Column("UserSettingsUserID")
           .Where($"UserOrganisation='{ddlUserOrganisations.SelectedValue}'");
