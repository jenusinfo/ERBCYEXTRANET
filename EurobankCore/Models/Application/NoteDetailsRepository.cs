...
namespace Eurobank.Models.Application
{
	public class NoteDetailsRepository
    {
        ...
        public NoteDetailsRepository(IPageRetriever pageRetriever, IPageDataContextRetriever pageDataContextRetriever)
        {
        ...
        public Task<IEnumerable<CMS.DocumentEngine.Types.Eurobank.NoteDetails>> GetNoteDetailsAsync(string nodeAliasPath, CancellationToken cancellationToken)
        {
            return pageRetriever.RetrieveAsync<CMS.DocumentEngine.Types.Eurobank.NoteDetails>(
                query => query
                    .Path(nodeAliasPath, PathTypeEnum.Children)
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(NoteDetails)}|{nameof(GetNoteDetailsAsync)}|{nodeAliasPath}")
                    // Include path dependency to flush cache when a new child page is created or page order is changed.
                    .Dependencies((_, builder) => builder.PagePath(nodeAliasPath, PathTypeEnum.Children).PageOrder()),
                cancellationToken);
        }
        public IEnumerable<CMS.DocumentEngine.Types.Eurobank.NoteDetails> GetNoteDetails(int apllicationID)
        {
            ...
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.NoteDetails>(
                query => query
                    .Path(apllicationDetails.NodeAliasPath, PathTypeEnum.Children)
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(NoteDetailsRepository)}|{nameof(NoteDetails)}|{apllicationDetails.NodeAliasPath}")
                    // Include path dependency to flush cache when a new child page is created or page order is changed.
        ...
        public CMS.DocumentEngine.Types.Eurobank.NoteDetails GetNoteDetail(int NoteDetailsID)
        {
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.NoteDetails>(
                query => query
                    .OrderBy("NodeOrder")
                    .WhereEquals("NoteDetailsID", NoteDetailsID)
               ).FirstOrDefault();
        }
    }
}
