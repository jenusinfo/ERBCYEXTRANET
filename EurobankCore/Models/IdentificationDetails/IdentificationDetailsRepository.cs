...
{
	public class IdentificationDetailsRepository
	{
        ...
        public IdentificationDetailsRepository(IPageRetriever pageRetriever, IPageDataContextRetriever pageDataContextRetriever)
        {
        ...
        /// </summary>
        /// <param name="nodeAliasPath">The node alias path of the home in the content tree.</param>
        public IEnumerable<CMS.DocumentEngine.Types.Eurobank.IdentificationDetails> GetIdentifications(string nodeAliasPath)
        {
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.IdentificationDetails>(
                query => query
                    .Path(nodeAliasPath, PathTypeEnum.Children)
                    .OrderBy("NodeOrder"),
                cache => cache
                    .Key($"{nameof(RegistriesRepository)}|{nameof(GetIdentifications)}|{nodeAliasPath}")
                    // Include path dependency to flush cache when a new child page is created or page order is changed.
                    .Dependencies((_, builder) => builder.PagePath(nodeAliasPath, PathTypeEnum.Children).PageOrder()));
        }
        ...
        public CMS.DocumentEngine.Types.Eurobank.IdentificationDetails GetIdentificationDetailsByID(int identificationDetailsID)
        {
            return pageRetriever.Retrieve<CMS.DocumentEngine.Types.Eurobank.IdentificationDetails>(
                query => query
                    .OrderBy("NodeOrder")
                    .WhereEquals("IdentificationDetailsID", identificationDetailsID)
               ).FirstOrDefault();
        }
    }
}
